...
// Returns true if 'curr' frame can be decoded independently of 'prev' canvas.
...
static int ReadSubImage(GifFileType* gif, WebPPicture* const prev,
                        int is_first_frame, size_t iframe_interval,
                        size_t* const count_since_iframe,
  ...
  *gif_rect = rect;
  ...
  if (is_first_frame || IsIndependentFrame(curr, &rect, prev)) {
    *count_since_iframe = 0;
  } else {
    ++*count_since_iframe;
    if (*count_since_iframe > iframe_interval) {
      MakeIndependentFrame(curr, prev, &rect);
      *count_since_iframe = 0;
    }
  }

  if (*count_since_iframe == 0) {
...
static void Help(void) {
  ...
  printf("  -imax <int> ............ Max distance between I-frames\n");
...
int main(int argc, const char *argv[]) {
  ...
  size_t count_since_iframe = 0;
  // Distance between independently decodable frames.
  size_t iframe_interval = 9;
  int done;
  int c;
  int quiet = 0;
  WebPConfig config;
  ...
  for (c = 1; c < argc; ++c) {
    ...
    } else if (!strcmp(argv[c], "-imax") && c < argc - 1) {
      iframe_interval = strtoul(argv[++c], NULL, 0);
  ...
  do {
    ...
    switch (type) {
      case IMAGE_DESC_RECORD_TYPE: {
        ...
        if (!ReadSubImage(gif, &previous, is_first_frame, iframe_interval,
                          &count_since_iframe, &gif_rect, &sub_image, &frame,
                          &current)) {
          goto End;
        }
