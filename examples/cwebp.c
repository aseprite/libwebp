...
// Stores 'profile' to 'payload->bytes', returns false on allocation error.
static int StoreProfile(const char* profile, size_t profile_len,
                        MetadataPayload* const payload) {
  payload->bytes = (uint8_t*)malloc(profile_len);
  if (payload->bytes == NULL) return 0;
  payload->size = profile_len;
  memcpy(payload->bytes, profile, profile_len);
  return 1;
}
...
// On success the raw buffer is returned with its length will be equivalent to
...
static const PNGMetadataMap kPNGMetadataMap[] = {
  // http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/PNG.html#TextualData
  // See also: ExifTool on CPAN.
  { "Raw profile type exif", ProcessRawProfile, METADATA_OFFSET(exif) },
  { "Raw profile type xmp",  ProcessRawProfile, METADATA_OFFSET(xmp) },
  // XMP Specification Part 3, Section 3 #PNG
  { "XML:com.adobe.xmp",     StoreProfile,      METADATA_OFFSET(xmp) },
  { NULL, NULL, 0 },
};
...
static int ExtractMetadataFromPNG(png_structp png,
  ...
  for (p = 0; p < 2; ++p)  {
    ...
    for (i = 0; i < num; ++i, ++text) {
      const PNGMetadataMap* iter = kPNGMetadataMap;
      for (iter = kPNGMetadataMap; iter->name != NULL; ++iter) {
        if (!strcmp(text->key, iter->name)) {
          MetadataPayload* const payload =
              (MetadataPayload*)((uint8_t*)metadata + iter->storage_offset);
          ...
          } else if (!iter->process(text->text, text_length, payload)) {
    ...
    ((1 << 8) | (5 << 0))
      ...
      if (png_get_iCCP(png, info,
        ...
        if (!StoreProfile((const char*)profile, len, &metadata->iccp)) return 0;
      }
    }
  }
...
static int ReadPNG(FILE* in_file, WebPPicture* const pic, int keep_alpha,
 ...
 Error:
  ...
  if (!ExtractMetadataFromPNG(png, info, end_info, metadata)) {
