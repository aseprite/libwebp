...
static void PrintFullLosslessInfo(const WebPAuxStats* const stats) {
  fprintf(stderr, "Lossless size: %d\n", stats->lossless_size);
  if (stats->lossless_features) {
    fprintf(stderr, "  * Lossless features used:");
    if (stats->lossless_features & 1) fprintf(stderr, " CROSS-COLOR-TRANSFORM");
    if (stats->lossless_features & 2) fprintf(stderr, " SUBTRACT-GREEN");
    if (stats->lossless_features & 4) fprintf(stderr, " PREDICTION");
...
static void PrintExtraInfoLossy(const WebPPicture* const pic, int short_output,
  ...
  if (short_output) {
    ...
    if (stats->lossless_size > 0) PrintFullLosslessInfo(stats);
  }
