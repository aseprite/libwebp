...
//
//  Compile with:     gcc -o webp2png webp2png.c -lwebpdecode
//
...
static int ReadFile(const char* filename, void** data, uint32_t* data_size) {
  int ok;
...
int main(int argc, const char *argv[]) {
...

  {
    ...
    if (!ReadFile(in_file, &data, &data_size)) return -1;
    ...
    if (status != VP8_STATUS_OK) goto End;
 ...
 End:
  ...
  if (out_file) {
    ...
    SaveOutput(output_buffer, out_file);
  } else {
