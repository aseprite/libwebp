...
//  WebP-container.
//
//  Compile with: gcc -o png2webp png2webp.c -lexperimental -lwebpmux -lwebp
...
int main(int argc, const char *argv[]) {
  ...
  // ReadInput, Encode and SaveOutput.
  for (c = 1; c < argc; ++c) {
    if (!strcmp(argv[c], "-h") || !strcmp(argv[c], "-help")) {
      ...
      alpha_q = strtol(argv[++c], NULL, 0);
#ifndef ZLIB_METHOD_ONLY
    } else if (!strcmp(argv[c], "-alpha_method") && c < argc - 1) {
      alpha_method = strtol(argv[++c], NULL, 0);
#endif
  ...
  // TODO: Handle Alpha Encoding inside WebPEncode.
  ...
  if (keep_alpha && picture.a) {
    if (!EncodeAlpha(picture.a, picture.width, picture.height, picture.a_stride,
                     alpha_q, alpha_method,
                     &compressed_alpha, &compressed_alpha_size)) {
      fprintf(stderr, "Error! Cannot encode Alpha\n");
      goto Error;
    }
  }
  ...
  if (out_file) {
    ...
    // Create an instance of Mux object from encoded WebP data (compressed_rgb).
    // TODO: Move most of this Mux handling code to Mux library and reduce the
    // code complexity from application code like this.
    mux_single = WebPMuxCreate(compressed_rgb, compressed_rgb_size, 0);
    if (mux_single == NULL) {
      fprintf(stderr, "Error! Cannot save image '%s'\n", out_file);
      goto Error;
    }
    err = WebPMuxGetImage(mux_single, &rgb_data, &rgb_size, NULL, NULL);
    ...
    mux = WebPMuxNew();
    if (mux == NULL) {
      fprintf(stderr, "Error! Cannot save WebP image '%s'\n", out_file);
      goto Error;
    }

    err = WebPMuxSetImage(mux, rgb_data, rgb_size,
                          compressed_alpha, compressed_alpha_size, 0);
    if (err != WEBP_MUX_OK) {
      fprintf(stderr, "Error! Cannot save WebP image '%s'\n", out_file);
      goto Error;
    }

    err = WebPMuxAssemble(mux, &out_data, &out_data_size);
    if (err != WEBP_MUX_OK) {
      free(out_data);
      fprintf(stderr, "Error! Cannot save image '%s'\n", out_file);
      goto Error;
    ...
    if (fwrite(out_data, out_data_size, 1, out) != 1) {
      free(out_data);
      fprintf(stderr, "Error! Cannot save image '%s'\n", out_file);
      goto Error;
    }
