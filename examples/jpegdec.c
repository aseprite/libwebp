...
static void SaveMetadataMarkers(j_decompress_ptr dinfo) {
  ...
  jpeg_save_markers(dinfo, JPEG_APP0 + 1, max_marker_length);  // Exif/XMP
  jpeg_save_markers(dinfo, JPEG_APP0 + 2, max_marker_length);  // ICC profile
}
...
// Returns true in all cases except for memory errors and warns for corrupt
// profiles.
    ...
    if (marker->marker == JPEG_APP0 + 2 &&
      ...
      if (segment_size == 0 || count == 0 || seq == 0) {
        fprintf(stderr, "[ICCP] size (%zu) / count (%d) / sequence number (%d)"
                        " cannot be 0! Discarding profile.\n",
