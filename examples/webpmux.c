...
static WebPMuxError DisplayInfo(const WebPMux* mux) {
  ...
  if (flag & ANIMATION_FLAG) {
    ...
    if (nFrames > 0) {
      int i;
      fprintf(stderr, "No.: x_offset y_offset duration image_size");
      ...
      for (i = 1; i <= nFrames; i++) {
        ...
        fprintf(stderr, "%3d: %8d %8d %8d %10u",
  ...
  if (flag & TILE_FLAG) {
    ...
    if (nTiles > 0) {
      ...
      fprintf(stderr, "No.: x_offset y_offset image_size");
      if (flag & ALPHA_FLAG) fprintf(stderr, " alpha_size");
      fprintf(stderr, "\n");
      for (i = 1; i <= nTiles; i++) {
        WebPData image, alpha;
