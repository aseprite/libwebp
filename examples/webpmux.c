...
// -----------------------------------------------------------------------------
//
//  simple command-line to create WebP container from given set of tile images.
...
static int ReadFile(WebPMux* mux, const char* const filename) {
  uint32_t size = 0;
  void* data = NULL;
  WebPMuxError err;

  if (!ReadData(filename, &data, &size)) return 0;
  err = WebPMuxCreate(mux, (const uint8_t*)data, size);
  free(data);
  if (err == WEBP_MUX_OK) return 1;
  fprintf(stderr, "Failed to create mux object from file %s. Error: %d\n",
          filename, err);
  return 0;
}
...
static int ReadImageData(const char* filename, int image_index,
  ...
  if (!ReadData(filename, &data, &size)) {
    WebPMuxDelete(mux);
    return 0;
  }
...
static int ValidateCommandLine(int argc, const char* argv[],
  ...
  *num_feature_args = 0;
  ...
  num_get_args = CountOccurences(argv, argc, "-get");
  num_set_args = CountOccurences(argv, argc, "-set");
  num_strip_args = CountOccurences(argv, argc, "-strip");
  num_info_args = CountOccurences(argv, argc, "-info");
  num_help_args = CountOccurences(argv, argc, "-help");
  num_outfile_args = CountOccurences(argv, argc, "-o");
...
static int GetFrameTile(WebPMux* mux, WebPMuxConfig* config, int isFrame) {
  ...
  if (isFrame) {
    err = WebPMuxGetFrame(mux, num, &data, &size, &x_offset, &y_offset,
                          &duration);
    if (err != WEBP_MUX_OK) {
      ERROR_GOTO3("ERROR#%d: Could not get frame %ld.\n", err, num, ErrGet);
    }
  } else {
    err = WebPMuxGetTile(mux, num, &data, &size, &x_offset, &y_offset);
    if (err != WEBP_MUX_OK) {
      ERROR_GOTO3("ERROR#%d: Could not get frame %ld.\n", err, num, ErrGet);
    }
  }

  mux_single = WebPMuxNew(1);
  if (mux_single == NULL) {
    err = WEBP_MUX_MEMORY_ERROR;
    ERROR_GOTO2("ERROR#%d: Could not allocate a mux object.\n", err, ErrGet);
  }
  err = WebPMuxAddNamedData(mux_single, 0, "image", data, size);
  if (err != WEBP_MUX_OK) {
      ...
      switch(feature->type_) {
        case FEATURE_FRM:
          for (index = 0; index < feature->arg_count_; ++index) {
            if (feature->args_[index].subtype_ == SUBTYPE_LOOP) {
              ...
              ok = ReadImageData(feature->args_[index].filename_, 1,
                                 &data, &size);
              if (!ok) goto Err2;
              ...
              err = WebPMuxAddFrame(mux, 0, data, size, x_offset, y_offset,
                                    duration);
              free((void*)data);
              if (err != WEBP_MUX_OK) {
        ...
        case FEATURE_TILE:
          for (index = 0; index < feature->arg_count_; ++index) {
            ...
            err = WebPMuxAddTile(mux, 0, data, size, x_offset, y_offset);
            free((void*)data);
            if (err != WEBP_MUX_OK) {
        ...
        case FEATURE_ICCP:
          ...
          ok = ReadData(feature->args_[0].filename_, (void**)&data,
                        &size);
          if (!ok) goto Err2;
          err = WebPMuxSetColorProfile(mux, data, size);
          free((void*)data);
          if (err != WEBP_MUX_OK) {
        ...
        case FEATURE_XMP:
          ...
          ok = ReadData(feature->args_[0].filename_, (void**)&data,
                        &size);
          if (!ok) goto Err2;
          err = WebPMuxSetMetadata(mux, data, size);
          free((void*)data);
          if (err != WEBP_MUX_OK) {
