...
static HRESULT ExtractICCP(IWICImagingFactory* const pFactory,
  ...
  if (SUCCEEDED(hr)) {
    UINT c;
    IFS(IWICBitmapFrameDecode_GetColorContexts(pFrame, count, ppColorContext, &c));
    for (i = 0; SUCCEEDED(hr) && i < c; ++i) {
      WICColorContextType type;
      IFS(IWICColorContext_GetType(ppColorContext[i], &type));
      if (SUCCEEDED(hr) && type == WICColorContextProfile) {
        UINT size;
        IFS(IWICColorContext_GetProfileBytes(ppColorContext[i], 0, NULL, &size));
        if (size > 0) {
  ...
  for (i = 0; i < count; ++i) IUnknown_Release(ppColorContext[i]);
  free(ppColorContext);
  return hr;
}
