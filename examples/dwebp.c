...
#define EXTRA_DATA_OFFSET (8 + 2 + 12 * NUM_IFD_ENTRIES + 4)
...
static int WriteTIFF(FILE* fout, const WebPDecBuffer* const buffer) {
  ...
  uint8_t tiff_header[TIFF_HEADER_SIZE] = {
      ...
      0x02, 0x01, 3, 0, 4, 0, 0, 0,                //  34: BitsPerSample: 8888
      ...
      0x52, 0x01, 3, 0, 1, 0, 0, 0,                // 178: ExtraSamples: rgbA
          has_alpha ? 1 : 0, 0, 0, 0,
  ...
  PutLE32(tiff_header + 106 + 8, height);                  // rows-per-strip
  PutLE32(tiff_header + 118 + 8, height * bytes_per_px);   // strip byte count
