  ...
  if (format >= RGB && format <= rgbA_4444) {
    const int bpp = (format == RGB || format == BGR) ? 3
                  : (format == RGBA_4444 || format == rgbA_4444 ||
                     format == RGB_565) ? 2
                  : 4;
    uint64_t stride = bpp * w + 7;   // <- just for exercising
    ...
    uint8_t* tmp;
    uint64_t stride = w + 3;
    uint64_t uv_stride = (w + 1) / 2 + 13;
    uint64_t total_size = stride * h * (has_alpha ? 2 : 1)
                        + 2 * uv_stride * (h + 1) / 2;
...
int main(int argc, const char *argv[]) {
  ...
  for (c = 1; c < argc; ++c) {
    ...
    if (!strcmp(argv[c], "-h") || !strcmp(argv[c], "-help")) {
      ...
      parse_error |= (use_external_memory > 2);
 ...
 Error:
    ...
    if (use_external_memory > 0) {
      external_buffer = AllocateExternalBuffer(&config, format,
                                               use_external_memory);
      if (external_buffer == NULL) goto Error;
    }
