...
static void ZeroFillFrameRect(uint8_t* rgba, size_t rgba_stride, int x_offset,
...
static void CopyFrameRectangle(const uint8_t* src, uint8_t* dst, size_t stride,
...
static bool DumpFrame(const char filename[], const char dump_folder[],
  ...
  for (int y = 0; y < canvas_height; ++y) {
    fout.write((const char*)rgba + y * canvas_width * kNumChannels,
...
static bool ReadAnimatedWebP(const char filename[], const std::string& file_str,
  ...
  WebPIterator prev_iter = { 0 };
  WebPIterator curr_iter = { 0 };
