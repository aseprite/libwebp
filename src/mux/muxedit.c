...
static WebPMuxError MuxPushFrameTileInternal(
  ...
  // If given offsets are odd, gracefully convert them to even (offset - 1).
  x_offset -= (x_offset & 1);
  y_offset -= (y_offset & 1);
...
static WebPMuxError GetFrameTileInfo(const WebPChunk* const frame_tile_chunk,
  ...
  *y_offset = 2 * GetLE24(data->bytes_ + 3);
  if (is_frame) *duration = GetLE24(data->bytes_ + 12);
  return WEBP_MUX_OK;
}
...
static WebPMuxError GetImageCanvasWidthHeight(
    const WebPMux* const mux, uint32_t flags, int* width, int* height) {
  ...
  if (wpi->next_) {
    ...
    for (; wpi != NULL; wpi = wpi->next_) {
      int x_offset, y_offset, duration, width, height;
      const WebPMuxError err = GetImageInfo(wpi, &x_offset, &y_offset,
                                            &duration, &width, &height);
      const int max_x_pos = x_offset + width;
      const int max_y_pos = y_offset + height;
      if (err != WEBP_MUX_OK) return err;
