...
// Assigns given data to 'chunk' and sets 'chunk' as 1st chunk 'CHUNK_LIST'.
#define SET_CHUNK_IN_WPI(DATA, TAG, CHUNK_LIST)                                \
...
// This macro extracts image & alpha data from the given bitstream and then sets
// wpi.alpha_ and wpi.img_ appropriately.
#define SET_ALPHA_AND_IMAGE_CHUNKS(IMAGE_TAG, ALPHA)                           \
    ...
    if ((ALPHA).bytes_ != NULL) {                                              \
      SET_CHUNK_IN_WPI((ALPHA), kChunks[IDX_ALPHA].tag, wpi.alpha_);           \
    }                                                                          \
    SET_CHUNK_IN_WPI(image, (IMAGE_TAG), wpi.img_);
...
WebPMuxError WebPMuxSetImage(WebPMux* mux, const WebPData* bitstream,
  ...
  SET_ALPHA_AND_IMAGE_CHUNKS(image_tag, alpha);
...
WebPMuxError WebPMuxPushFrame(WebPMux* mux, const WebPMuxFrameInfo* frame,
  ...
  {
    int x_offset = frame->x_offset_;
    int y_offset = frame->y_offset_;
    ...
    WebPDataClear(&frame_tile);  // frame_tile owned by chunk now.
    if (err != WEBP_MUX_OK) goto Err;
...
#undef SET_CHUNK_IN_WPI
#undef SET_ALPHA_AND_IMAGE_CHUNKS
