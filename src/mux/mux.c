...
struct WebPMux {
  ...
  int         copy_data_;  // Value of 0 indicates data will NOT be copied to
                           // mux-chunks in subsequent calls, whereas
                           // value of 1 indicates data WILL be copied.
};

//-----------------------------------------------------------------------------
// Internal struct managment.
...
static int WebPMuxInit(WebPMux* const mux, int copy_data) {
  if (mux == NULL) return 0;
  memset(mux, 0, sizeof(*mux));
  mux->copy_data_ = copy_data;
  return 1;
}

WebPMux* WebPMuxNew(int copy_data) {
  WebPMux* const mux = (WebPMux*)malloc(sizeof(WebPMux));
  if (mux) WebPMuxInit(mux, copy_data);
  return mux;
}
...
WebPMuxError WebPMuxCreate(WebPMux* const mux, const uint8_t* data,
  ...
  while (data != end) {
    ...
    InitChunk(&chunk);
    err = RecordChunk(&chunk, data, mux_size, mux->copy_data_);
...
WebPMuxError WebPMuxSetMetadata(WebPMux* const mux, const uint8_t* data,
  ...
  return MuxSet(mux, META_ID, 1, data, size, NULL, mux->copy_data_);
}
...
WebPMuxError WebPMuxSetColorProfile(WebPMux* const mux, const uint8_t* data,
  ...
  return MuxSet(mux, ICCP_ID, 1, data, size, NULL, mux->copy_data_);
}
...
WebPMuxError WebPMuxSetLoopCount(WebPMux* const mux, uint32_t loop_count) {
  ...
  err = WebPMuxAddChunk(mux, 1, kChunks[LOOP_ID].chunkTag, data,
                        kChunks[LOOP_ID].chunkSize, NULL, mux->copy_data_);
  free(data);
  return err;
}
...
WebPMuxError WebPMuxAddFrame(WebPMux* const mux, uint32_t nth,
  ...
  return WebPMuxAddFrameTileInternal(mux, nth, data, size, x_offset, y_offset,
                                     duration, kChunks[FRAME_ID].chunkTag);
}
  ...
  if (image_chunk->next_) {
    ...
    for (; image_chunk != NULL; image_chunk = image_chunk->next_) {
      ...
      if (image_info != NULL) {
        ...
        if (max_x_pos < image_info->x_offset_) {  // Overflow occured.
          return WEBP_MUX_INVALID_ARGUMENT;
        }
        if (max_y_pos < image_info->y_offset_) {  // Overflow occured.
          return WEBP_MUX_INVALID_ARGUMENT;
        }
        if (max_x_pos > max_x) {
          max_x = max_x_pos;
...
static WebPMuxError CreateVp8xChunk(WebPMux* const mux) {
  ...
  err = GetImageCanvasHeightWidth(mux, &width, &height, flags);
  if (err != WEBP_MUX_OK) return err;
