...
static void MarkError2(WebPAnimEncoder* const enc,
  ...
  if (snprintf(enc->error_str_, ERROR_STR_MAX_LENGTH, format, value) < 0) {
    assert(0);  // FIX ME!
  }
}

static void MarkError(WebPAnimEncoder* const enc, const char* str) {
  MarkError2(enc, str, 0 /* dummy */);
}
...
static int CacheFrame(WebPAnimEncoder* const enc,
 ...
 End:
  if (!ok || frame_skipped) {
    ...
    if (!ok) {
      MarkError2(enc, "ERROR adding frame. WebPEncodingError: %d.", error_code);
    }
  }
...
static int FlushFrames(WebPAnimEncoder* const enc) {
  while (enc->flush_count_ > 0) {
    ...
    if (err != WEBP_MUX_OK) {
      MarkError2(enc, "ERROR adding frame. WebPMuxError: %d.", err);
      return 0;
    }
...
int WebPAnimEncoderAssemble(WebPAnimEncoder* enc, WebPData* webp_data) {
 ...
 Err:
  MarkError2(enc, "ERROR assembling WebP: %d", err);
  return 0;
}
