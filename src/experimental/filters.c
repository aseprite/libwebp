...
static void AverageFilter(const uint8_t* data, int width, int height,
                          int bpp, int stride, uint8_t* filtered_data) {
  int h;
  SANITY_CHECK(data, filtered_data);
  ...
  for (h = 1; h < height; ++h) {
    ...
    const uint8_t* const prev_line = scan_line - stride;
    for (w = 0; w < bpp; ++w) {
      // Predict from top if left position is unavailable.
      out[w] = scan_line[w] - prev_line[w];
    }
    for (w = bpp; w < width * bpp; ++w) {
      const uint8_t predictor = (scan_line[w - bpp] + prev_line[w] + 1) >> 1;
      out[w] = scan_line[w] - predictor;
    }
  }
}

static void AverageUnfilter(const uint8_t* data, int width, int height,
                            int bpp, int stride, uint8_t* recon_data) {
  int h;
  SANITY_CHECK(data, recon_data);
  ...
  for (h = 1; h < height; ++h) {
    ...
    for (w = bpp; w < width * bpp; ++w) {
      const uint8_t predictor = (out[w - bpp] + out_prev_line[w] + 1) >> 1;
      out[w] = scan_line[w] + predictor;
    }
  }
}
...
const WebPFilterFunc WebPFilters[WEBP_FILTER_LAST] = {
    ...
    AverageFilter,     // WEBP_FILTER_AVERAGE
...
const WebPFilterFunc WebPUnfilters[WEBP_FILTER_LAST] = {
    ...
    AverageUnfilter,     // WEBP_FILTER_AVERAGE
