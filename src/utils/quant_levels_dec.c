...
#define DFIX 4   // precision for dithering
#else
#define DFIX 0
#endif
...
#define MAX_LUT ((1 << (8 + LFIX)) - 1)  // look-up table size
...
typedef struct {
  ...
  int16_t correction_[1 + 2 * MAX_LUT];
} SmoothParams;
...
static void v_filter(SmoothParams* const p) {
  ...
  // We replicate edges, as it's somewhat easier as boundary condition.
...
static void h_filter(SmoothParams* const p) {
...
static void apply_filter(SmoothParams* const p) {
  const uint16_t* const average = p->average_;
  const int w = p->width_;
  const int16_t* const correction = p->correction_ + MAX_LUT;
#if defined(USE_DITHERING)
...
static void InitLUT(int min_dist, int16_t* const lut) {
  // The correction curve is:
  //   f(x) = x for x < threshold1
  //   f(x) = 0 for x > threshold2
  // and a linear interpolation for range x=[threshold1, threshold2]
...
static int InitParams(uint8_t* const data, int width, int height,
  ...
  const size_t total_size = size_scratch_m + size_m;
  ...
  p->min_level_dist_ = 255;
  for (last_level = -1, i = 0; i < 256; ++i) {
    if (counts[i]) {
      ++p->num_levels_;
      if (last_level >= 0) {
...
//  printf("# %d Levels, min=%d max=%d, min_dist=%d\n",
//         p->num_levels_, p->min_, p->max_, p->min_level_dist_);

  InitLUT(p->min_level_dist_, p->correction_ + MAX_LUT);
...
int WebPDequantizeLevels(uint8_t* const data, int width, int height) {
  ...
  if (!InitParams(data, width, height, kRadius, &p)) return 0;
  for (; p.row_ < p.height_; ++p.row_) {
    v_filter(&p);  // accumulate average of input
    // Need to wait few rows to prime the filter, before emitting some output.
    if (p.row_ >= p.radius_) {
      h_filter(&p);
      apply_filter(&p);
    }
