...
static void ExportRowMIPS_dsp_r2(WebPRescaler* const wrk, int x_out) {
  if (wrk->y_accum <= 0) {
    ...
    // otherwise use C code
    if ((wrk->fxy_scale >> 32) == 0) {
      int temp0, temp1, temp3, temp4, temp5, temp6, temp7, rest;
      int32_t* loop_end;
      const int temp2 = (int)(wrk->fxy_scale);
      uint8_t* dst_t = (uint8_t*)dst;
      int32_t* irow_t = (int32_t*)irow;
      int32_t* frow_t = (int32_t*)frow;
      temp0 = x_out_max - x_out;
      loop_end = frow_t + (temp0 & ~1);
      rest = temp0 & 1;
      __asm__ volatile(
        ".set             push                                    \n\t"
        ".set             noreorder                               \n\t"
        "beq              %[frow_t],   %[loop_end],   1f          \n\t"
        " nop                                                     \n\t"
      "0:                                                         \n\t"
        "lw               %[temp0],    0(%[frow_t])               \n\t"
        "lw               %[temp1],    0(%[irow_t])               \n\t"
        ...
        "addiu            %[frow_t],   %[frow_t],     8           \n\t"
        "addiu            %[dst_t],    %[dst_t],      2           \n\t"
        "addiu            %[irow_t],   %[irow_t],     8           \n\t"
