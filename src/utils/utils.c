...
static void PrintMemInfo() {
  ...
  fprintf(stderr, "total_mem: %llu\n", (uint64_t)total_mem);
  fprintf(stderr, "high-water mark: %llu\n", (uint64_t)high_water_mark);
...
static void AddMem(void* ptr, size_t size) {
  if (ptr != NULL) {
    ...
    fprintf(stderr, "Mem: %llu (+%llu)\n", (uint64_t)total_mem, (uint64_t)size);
#endif
...
#if defined(PRINT_MEM_TRAFFIC)
    fprintf(stderr, "Mem: %llu (-%llu)\n",
            (uint64_t)total_mem, (uint64_t)block->size_);
#endif
    free(block);
  }
}
