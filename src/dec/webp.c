...
VP8StatusCode WebPParseHeaders(WebPHeaderStructure* const headers) {
  ...
  if (status == VP8_STATUS_OK || status == VP8_STATUS_NOT_ENOUGH_DATA) {
    // TODO(jzern): proper support of animation frames will require API additions.
    if (has_animation) {
...
static VP8StatusCode GetFeatures(const uint8_t* const data, size_t data_size,
  ...
  { // Only parse enough of the data to retrieve the features.
    int has_animation = 0;
    const VP8StatusCode status = ParseHeadersInternal(
        data, data_size, &features->width, &features->height,
        &features->has_alpha, &has_animation, NULL);
    features->has_animation = has_animation;
    return status;
  }
}
