...
VP8StatusCode WebPParseOptionalChunks(const uint8_t** data, uint32_t* data_size,
  ...
  *bytes_skipped = 0;
  ...
  while (1) {
    ...
    const uint32_t bytes_skipped_header = RIFF_HEADER_SIZE +  // RIFFnnnnWEBP.
                                          CHUNK_HEADER_SIZE +  // VP8x header.
...
VP8StatusCode WebPParseHeaders(const uint8_t** data, uint32_t* data_size,
  ...
  *bytes_skipped = 0;
  ...
  if (riff_size > 0) {  // RIFF header was found.
    bytes_skipped_tmp += RIFF_HEADER_SIZE;
  }
  ...
  if (vp8x_skip_size > 0) {
    bytes_skipped_tmp += vp8x_skip_size;
    ...
    bytes_skipped_tmp += optional_data_size;
  }
  ...
  if (vp8_skip_size > 0) {
    *vp8_size = vp8_size_tmp;
    bytes_skipped_tmp += vp8_skip_size;
  }

  *data = buf;
  *data_size = buf_size;
  *bytes_skipped = bytes_skipped_tmp;
  return VP8_STATUS_OK;
}
