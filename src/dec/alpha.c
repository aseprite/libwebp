...
static int ALPHDecode(VP8Decoder* const dec, int row, int num_rows) {
  ...
  if (alph_dec->method_ == ALPHA_NO_COMPRESSION) {
    memcpy(dec->alpha_plane_, dec->alpha_data_, dec->alpha_data_size_);
    dec->is_alpha_decoded_ = 1;
  } else {  // alph_dec->method_ == ALPHA_LOSSLESS_COMPRESSION
    WebPUnfilterFunc unfilter_func = WebPUnfilters[alph_dec->filter_];
    const VP8LDecoder* const vp8l_dec = alph_dec->vp8l_dec_;
    const int width = alph_dec->width_;
    const int height = alph_dec->height_;
    uint8_t* output = (uint8_t*)vp8l_dec->io_->opaque;
    assert(vp8l_dec != NULL && vp8l_dec->io_ != NULL);
    ...
    if (row + num_rows == dec->pic_hdr_.height_) dec->is_alpha_decoded_ = 1;
  }
  return 1;
}
