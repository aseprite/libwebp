...
static WEBP_INLINE void GetBaseDelta(const __m128i* const p1,
  ...
  const __m128i p0s = _mm_xor_si128(*p0, sign_bit);
  const __m128i q0s = _mm_xor_si128(*q0, sign_bit);
  // beware of addition order, for saturation!
  const __m128i v0 = _mm_subs_epi8(p1s, q1s);   // p1 - q1
  const __m128i v1 = _mm_subs_epi8(q0s, p0s);   // q0 - p0
  const __m128i v2 = _mm_adds_epi8(v0, v1);     // p1 - q1 + 1 * (q0 - p0)
  const __m128i v3 = _mm_adds_epi8(v2, v1);     // p1 - q1 + 2 * (q0 - p0)
  const __m128i v4 = _mm_adds_epi8(v3, v1);     // p1 - q1 + 3 * (q0 - p0)
  *delta = v4;
}
...
static WEBP_INLINE void DoSimpleFilter(__m128i* const p0, __m128i* const q0,
  ...
  *q0 = _mm_subs_epi8(q0s, v4);        // q0 -= v4
  *q0 = _mm_xor_si128(*q0, sign_bit);

  SignedShift8b(&v3);                  // v3 >> 3
  *p0 = _mm_adds_epi8(p0s, v3);        // p0 += v3
...
static WEBP_INLINE void Update2Pixels(__m128i* const pi, __m128i* const qi,
                                      const __m128i* const a_lo,
                                      const __m128i* const a_hi) {
  const __m128i a_lo_2 = _mm_srai_epi16(*a_lo, 7);
  const __m128i a_hi_2 = _mm_srai_epi16(*a_hi, 7);
  const __m128i delta = _mm_packs_epi16(a_lo_2, a_hi_2);
  *pi = _mm_add_epi8(*pi, delta);
  *qi = _mm_sub_epi8(*qi, delta);
}

static WEBP_INLINE void NeedsFilter(const __m128i* p1, const __m128i* p0,
                                    const __m128i* q0, const __m128i* q1,
                                    int thresh, __m128i *mask) {
...
static WEBP_INLINE void DoFilter2(const __m128i* const p1, __m128i* const p0,
                                  __m128i* const q0, const __m128i* const q1,
                                  int thresh) {
  __m128i a, mask;
...
static WEBP_INLINE void DoFilter4(__m128i* const p1, __m128i* const p0,
  ...
  *q0 = _mm_subs_epi8(*q0, t3);      // q0 -= t3
  ...
  // this is equivalent to (a + 1) >> 1 signed
