...
// input m = MM_ABS(p1, p0). Updates with the two other abs() pairs.
#define MAX_DIFF_PARTIAL(p3, p2, p1, m) do {                                   \
...
static void VFilter16i(uint8_t* p, int stride,
                       int thresh, int ithresh, int hev_thresh) {
  int k;
  __m128i mask, p1, p0;   // loop invariants

  {  // prologue
    __m128i p3, p2;
    LOAD_H_EDGES4(p, stride, p3, p2, p1, p0);
    mask = MM_ABS(p1, p0);
    MAX_DIFF_PARTIAL(p3, p2, p1, mask);   // compute partial mask
  }

  for (k = 3; k > 0; --k) {
    __m128i tmp_mask;
   __m128i q0, q1, tmp1, tmp2;
    p += 4 * stride;

    // Load q0, q1, q2, q3
    LOAD_H_EDGES4(p, stride, q0, q1, tmp1, tmp2);
    tmp_mask = MM_ABS(tmp1, tmp2);   // will be in the next loop invariant
    mask = _mm_max_epu8(mask, tmp_mask);
    MAX_DIFF_PARTIAL(q0, q1, tmp1, mask);
    ...
    // rotate samples and mask (loop invariants)
    p1 = tmp1;
    p0 = tmp2;
    mask = tmp_mask;
    MAX_DIFF_PARTIAL(q0, q1, p1, mask);   // update mask
  }
}
...
static void HFilter16i(uint8_t* p, int stride,
  ...
  {  // prologue
    __m128i p3, p2;
    Load16x4(p, p + 8 * stride, stride, &p3, &p2, &p1, &p0);  // p3, p2, p1, p0
    mask = MM_ABS(p1, p0);
    MAX_DIFF_PARTIAL(p3, p2, p1, mask);   // compute partial mask
  }
  ...
  for (k = 3; k > 0; --k) {
   ...
   __m128i q0, q1, tmp1, tmp2;
    ...
    Load16x4(p, p + 8 * stride, stride, &q0, &q1, &tmp1, &tmp2);  // q0, q1, q2, q3
    tmp_mask = MM_ABS(tmp1, tmp2);   // will be in the next loop invariant
    mask = _mm_max_epu8(mask, tmp_mask);
    MAX_DIFF_PARTIAL(q0, q1, tmp1, mask);
