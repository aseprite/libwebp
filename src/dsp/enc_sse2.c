...
static WEBP_INLINE void DC16(uint8_t* dst, const uint8_t* left,
  ...
  const int DC = VP8HorizontalAdd8b(&top_row) + VP8HorizontalAdd8b(&left_row) + 16;
  Put16(DC >> 5, dst);
}
...
static int TTransform(const uint8_t* inA, const uint8_t* inB,
  ...
  // Load and combine inputs.
  {
    ...
    tmp_1 = _mm_unpackhi_epi8(inAB_1, zero);
    tmp_2 = _mm_unpacklo_epi8(inAB_2, zero);
    tmp_3 = _mm_unpackhi_epi8(inAB_3, zero);
  ...
  // are commutative) and subsequent transpose.
  {
