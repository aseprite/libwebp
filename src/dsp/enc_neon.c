...
static WEBP_INLINE uint8x8x4_t DistoTranspose4x4U8(uint8x8x4_t d4_in) {
  ...
  d2_tmp0 = vtrn_u8((d4_in.val[0]), (d4_in.val[1]));
  d2_tmp1 = vtrn_u8((d4_in.val[2]), (d4_in.val[3]));
  d2_tmp2 = vtrn_u16(vreinterpret_u16_u8(d2_tmp0.val[0]),
                     vreinterpret_u16_u8(d2_tmp1.val[0]));
  d2_tmp3 = vtrn_u16(vreinterpret_u16_u8(d2_tmp0.val[1]),
...
static WEBP_INLINE int16x8x4_t DistoTranspose4x4S16(int16x8x4_t q4_in) {
  ...
  q2_tmp0 = vtrnq_s16(q4_in.val[0], q4_in.val[1]);
  q2_tmp1 = vtrnq_s16(q4_in.val[2], q4_in.val[3]);
  q2_tmp2 = vtrnq_s32(vreinterpretq_s32_s16(q2_tmp0.val[0]),
                      vreinterpretq_s32_s16(q2_tmp1.val[0]));
  q2_tmp3 = vtrnq_s32(vreinterpretq_s32_s16(q2_tmp0.val[1]),
...
static WEBP_INLINE int16x8x4_t DistoHorizontalPass(const uint8x8x4_t d4_in) {
  ...
  q_a0 = vreinterpretq_s16_u16(vaddl_u8(d4_in.val[0], d4_in.val[2]));
  q_a1 = vreinterpretq_s16_u16(vaddl_u8(d4_in.val[1], d4_in.val[3]));
  q_a3 = vreinterpretq_s16_u16(vsubl_u8(d4_in.val[0], d4_in.val[2]));
  q_a2 = vreinterpretq_s16_u16(vsubl_u8(d4_in.val[1], d4_in.val[3]));
  // tmp[0] = a0 + a1
  // tmp[1] = a3 + a2
  // tmp[2] = a3 - a2
  // tmp[3] = a0 - a1
  q4_out.val[0] = vaddq_s16(q_a0, q_a1);
  q4_out.val[1] = vaddq_s16(q_a3, q_a2);
  q4_out.val[2] = vsubq_s16(q_a3, q_a2);
  q4_out.val[3] = vsubq_s16(q_a0, q_a1);
  return q4_out;
}

static WEBP_INLINE int16x8x4_t DistoVerticalPass(int16x8x4_t q4_in) {
  int16x8_t q_a0, q_a1, q_a2, q_a3;
  q_a0 = vaddq_s16(q4_in.val[0], q4_in.val[2]);
  q_a1 = vaddq_s16(q4_in.val[1], q4_in.val[3]);
  q_a2 = vsubq_s16(q4_in.val[1], q4_in.val[3]);
  q_a3 = vsubq_s16(q4_in.val[0], q4_in.val[2]);
...
static WEBP_INLINE int16x4x4_t DistoLoadW(const uint16_t* w ) {
  ...
  q_w07 = vld1q_u16(&w[0]);
  q_w8f = vld1q_u16(&w[8]);
  d4_w.val[0] = vget_low_s16(vreinterpretq_s16_u16(q_w07));
  d4_w.val[1] = vget_high_s16(vreinterpretq_s16_u16(q_w07));
  d4_w.val[2] = vget_low_s16(vreinterpretq_s16_u16(q_w8f));
  d4_w.val[3] = vget_high_s16(vreinterpretq_s16_u16(q_w8f));
  return d4_w;
}
...
static WEBP_INLINE int32x2_t DistoSum(const int16x8x4_t q4_in,
  ...
  //sum += w[ 8] * abs(b2);
  //sum += w[12] * abs(b3);
  q_sum0 = vmull_s16(d4_w.val[0], vget_low_s16(q4_in.val[0]));
  q_sum1 = vmull_s16(d4_w.val[1], vget_low_s16(q4_in.val[1]));
  q_sum2 = vmull_s16(d4_w.val[2], vget_low_s16(q4_in.val[2]));
  q_sum3 = vmull_s16(d4_w.val[3], vget_low_s16(q4_in.val[3]));
...
#define ConvertU32ToU8(SRC, DST) \
    (DST) = vreinterpret_u8_u32((SRC))
...
static int Disto4x4(const uint8_t* const a, const uint8_t* const b,
  ...
  ConvertU32ToU8(d_in_ab_0123, d4_in.val[0]);
  ConvertU32ToU8(d_in_ab_4567, d4_in.val[1]);
  ConvertU32ToU8(d_in_ab_89ab, d4_in.val[2]);
  ConvertU32ToU8(d_in_ab_cdef, d4_in.val[3]);

  // horizontal pass
  d4_in = DistoTranspose4x4U8(d4_in);
  q4_t = DistoHorizontalPass(d4_in);

  // load data w
  d4_w = DistoLoadW(w);

  // vertical pass
  q4_t = DistoTranspose4x4S16(q4_t);
  q4_t = DistoVerticalPass(q4_t);
  d_sum = DistoSum(q4_t, d4_w);
