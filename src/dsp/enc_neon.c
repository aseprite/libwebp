...
static int Disto4x4(const uint8_t* const a, const uint8_t* const b,
  ...
  uint32x2_t d_in_ab_0123 = { 0, 0 };
  uint32x2_t d_in_ab_4567 = { 0, 0 };
  uint32x2_t d_in_ab_89ab = { 0, 0 };
  uint32x2_t d_in_ab_cdef = { 0, 0 };
  uint8x8x2_t d2_tmp0, d2_tmp1;
  uint16x4x2_t d2_tmp2, d2_tmp3;
  uint8x8_t d_in0, d_in1, d_in2, d_in3;
  int16x8_t q_a0, q_a1, q_a2, q_a3;
  int16x8_t q_t0, q_t1, q_t2, q_t3;
  int16x8x2_t q2_tmp0, q2_tmp1;
  int32x4x2_t q2_tmp2, q2_tmp3;
  int16x8_t q_b0, q_b1, q_b2, q_b3;
  uint16x8_t q_w07, q_w8f;
  int16x4_t d_w0123, d_w4567, d_w89ab, d_wcdef;
  int32x4_t q_sum0, q_sum1, q_sum2, q_sum3;
  int32x2_t d_sum;

  d_in_ab_0123 = vld1_lane_u32((uint32_t*)(a + 0 * BPS), d_in_ab_0123, 0);
  d_in_ab_4567 = vld1_lane_u32((uint32_t*)(a + 1 * BPS), d_in_ab_4567, 0);
  d_in_ab_89ab = vld1_lane_u32((uint32_t*)(a + 2 * BPS), d_in_ab_89ab, 0);
  d_in_ab_cdef = vld1_lane_u32((uint32_t*)(a + 3 * BPS), d_in_ab_cdef, 0);

  d_in_ab_0123 = vld1_lane_u32((uint32_t*)(b + 0 * BPS), d_in_ab_0123, 1);
  d_in_ab_4567 = vld1_lane_u32((uint32_t*)(b + 1 * BPS), d_in_ab_4567, 1);
  d_in_ab_89ab = vld1_lane_u32((uint32_t*)(b + 2 * BPS), d_in_ab_89ab, 1);
  d_in_ab_cdef = vld1_lane_u32((uint32_t*)(b + 3 * BPS), d_in_ab_cdef, 1);
  ...
  d2_tmp0 = vtrn_u8(vreinterpret_u8_u32(d_in_ab_0123), vreinterpret_u8_u32(d_in_ab_4567));
  d2_tmp1 = vtrn_u8(vreinterpret_u8_u32(d_in_ab_89ab), vreinterpret_u8_u32(d_in_ab_cdef));
  d2_tmp2 = vtrn_u16(vreinterpret_u16_u8(d2_tmp0.val[0]), vreinterpret_u16_u8(d2_tmp1.val[0]));
  d2_tmp3 = vtrn_u16(vreinterpret_u16_u8(d2_tmp0.val[1]), vreinterpret_u16_u8(d2_tmp1.val[1]));

  d_in0 = vreinterpret_u8_u16(d2_tmp2.val[0]);
  d_in2 = vreinterpret_u8_u16(d2_tmp2.val[1]);
  d_in1 = vreinterpret_u8_u16(d2_tmp3.val[0]);
  d_in3 = vreinterpret_u8_u16(d2_tmp3.val[1]);

  // {a0, a1} = {in[0] + in[2], in[1] + in[3]}
  // {a3, a2} = {in[0] - in[2], in[1] - in[3]}
  q_a0 = vreinterpretq_s16_u16(vaddl_u8(d_in0, d_in2));
  q_a1 = vreinterpretq_s16_u16(vaddl_u8(d_in1, d_in3));
  q_a3 = vreinterpretq_s16_u16(vsubl_u8(d_in0, d_in2));
  q_a2 = vreinterpretq_s16_u16(vsubl_u8(d_in1, d_in3));

  // tmp[0] = a0 + a1
  // tmp[1] = a3 + a2
  // tmp[2] = a3 - a2
  // tmp[3] = a0 - a1
  q_t0 = vaddq_s16(q_a0, q_a1);
  q_t1 = vaddq_s16(q_a3, q_a2);
  q_t2 = vsubq_s16(q_a3, q_a2);
  q_t3 = vsubq_s16(q_a0, q_a1);

  // tmp a 048c, tmp b 048c
  // tmp a 159d, tmp b 159d
  // tmp a 26ae, tmp b 26ae
  // tmp a 37bf, tmp b 37bf
  //
  // transpose
  //
  // tmp a 0123, tmp b 0123
  // tmp a 4567, tmp b 4567
  // tmp a 89ab, tmp b 89ab
  // tmp a cdef, tmp b cdef
  //
  // Notice the middle values are transposed
  q2_tmp0 = vtrnq_s16(q_t0, q_t1);
  q2_tmp1 = vtrnq_s16(q_t2, q_t3);
  ...
  q_w07 = vld1q_u16(&w[0]);
  q_w8f = vld1q_u16(&w[8]);
  d_w0123 = vget_low_s16(vreinterpretq_s16_u16(q_w07));
  d_w4567 = vget_high_s16(vreinterpretq_s16_u16(q_w07));
  d_w89ab = vget_low_s16(vreinterpretq_s16_u16(q_w8f));
  d_wcdef = vget_high_s16(vreinterpretq_s16_u16(q_w8f));
  q_t0 = vreinterpretq_s16_s32(q2_tmp2.val[0]);
  q_t2 = vreinterpretq_s16_s32(q2_tmp2.val[1]);
  q_t1 = vreinterpretq_s16_s32(q2_tmp3.val[0]);
  q_t3 = vreinterpretq_s16_s32(q2_tmp3.val[1]);

  //a0 = tmp[0] + tmp[8];
  //a1 = tmp[4] + tmp[12];
  //a2 = tmp[4] - tmp[12];
  //a3 = tmp[0] - tmp[8];
  q_a0 = vaddq_s16(q_t0, q_t2);
  q_a1 = vaddq_s16(q_t1, q_t3);
  q_a2 = vsubq_s16(q_t1, q_t3);
  q_a3 = vsubq_s16(q_t0, q_t2);

  q_b0 = vaddq_s16(q_a0, q_a1);
  q_b1 = vaddq_s16(q_a3, q_a2);
  q_b2 = vabdq_s16(q_a3, q_a2);
  q_b3 = vabdq_s16(q_a0, q_a1);
  q_b0 = vabsq_s16(q_b0);
  q_b1 = vabsq_s16(q_b1);
  ...
  //sum += w[ 8] * abs(b2);
  //sum += w[12] * abs(b3);
  q_sum0 = vmull_s16(d_w0123, vget_low_s16(q_b0));
  q_sum1 = vmull_s16(d_w4567, vget_low_s16(q_b1));
  q_sum2 = vmull_s16(d_w89ab, vget_low_s16(q_b2));
  q_sum3 = vmull_s16(d_wcdef, vget_low_s16(q_b3));
  q_sum0 = vmlsl_s16(q_sum0, d_w0123, vget_high_s16(q_b0));
  q_sum1 = vmlsl_s16(q_sum1, d_w4567, vget_high_s16(q_b1));
  q_sum2 = vmlsl_s16(q_sum2, d_w89ab, vget_high_s16(q_b2));
  q_sum3 = vmlsl_s16(q_sum3, d_wcdef, vget_high_s16(q_b3));
