...
static WEBP_INLINE int16x8x4_t DistoHorizontalPass(const int16x8x4_t d4_in) {
  // {a0, a1} = {in[0] + in[2], in[1] + in[3]}
  // {a3, a2} = {in[0] - in[2], in[1] - in[3]}
  const int16x8_t q_a0 = vaddq_s16(d4_in.val[0], d4_in.val[2]);
  const int16x8_t q_a1 = vaddq_s16(d4_in.val[1], d4_in.val[3]);
  const int16x8_t q_a3 = vsubq_s16(d4_in.val[0], d4_in.val[2]);
  const int16x8_t q_a2 = vsubq_s16(d4_in.val[1], d4_in.val[3]);
  int16x8x4_t q4_out;
  // tmp[0] = a0 + a1
  // tmp[1] = a3 + a2
  // tmp[2] = a3 - a2
  ...
  q4_out.val[0] = vabsq_s16(q4_out.val[0]);
  q4_out.val[1] = vabsq_s16(q4_out.val[1]);
  return q4_out;
}
