...
#define MULT_FIX(A, B)                                                \
    vreinterpretq_u32_s32(vqrdmulhq_n_s32(vreinterpretq_s32_u32((A)), \
                                          (uint32_t)((B) >> 1)))
#else
#error "MULT_FIX/WEBP_RESCALER_RFIX need some more work"
#endif

static uint32x4_t Interpolate(const rescaler_t* frow,
                              const rescaler_t* irow,
                              uint32_t A, uint32_t B) {
  LOAD_32x4(frow, A0);
  LOAD_32x4(irow, B0);
  const uint64x2_t C0 = vmull_n_u32(vget_low_u32(A0), A);
  const uint64x2_t C1 = vmull_n_u32(vget_high_u32(A0), A);
  const uint64x2_t C2 = vmull_n_u32(vget_low_u32(B0), B);
  const uint64x2_t C3 = vmull_n_u32(vget_high_u32(B0), B);
  const uint64x2_t D0 = vaddq_u64(C0, C2);
  const uint64x2_t D1 = vaddq_u64(C1, C3);
...
static void RescalerExportRowExpand(WebPRescaler* const wrk) {
  ...
  if (wrk->y_accum == 0) {
    for (x_out = 0; x_out < max_span; x_out += 8) {
      ...
      const uint32x4_t B0 = MULT_FIX(A0, fy_scale);
      const uint32x4_t B1 = MULT_FIX(A1, fy_scale);
      const uint16x4_t C0 = vqmovn_u32(B0);
      const uint16x4_t C1 = vqmovn_u32(B1);
    ...
    for (x_out = 0; x_out < max_span; x_out += 8) {
      ...
      const uint32x4_t D0 = MULT_FIX(C0, fy_scale);
      const uint32x4_t D1 = MULT_FIX(C1, fy_scale);
      const uint16x4_t E0 = vqmovn_u32(D0);
      const uint16x4_t E1 = vqmovn_u32(D1);
...
static void RescalerExportRowShrink(WebPRescaler* const wrk) {
  ...
  if (yscale) {
    for (x_out = 0; x_out < max_span; x_out += 8) {
      ...
      const uint32x4_t A0 = MULT_FIX(in0, yscale);
      const uint32x4_t A1 = MULT_FIX(in1, yscale);
      const uint32x4_t B0 = vqsubq_u32(in2, A0);
      const uint32x4_t B1 = vqsubq_u32(in3, A1);
      const uint32x4_t C0 = MULT_FIX(B0, fxy_scale);
      const uint32x4_t C1 = MULT_FIX(B1, fxy_scale);
      const uint16x4_t D0 = vqmovn_u32(C0);
      const uint16x4_t D1 = vqmovn_u32(C1);
    ...
    for (x_out = 0; x_out < max_span; x_out += 8) {
      ...
      const uint32x4_t A0 = MULT_FIX(in0, fxy_scale);
      const uint32x4_t A1 = MULT_FIX(in1, fxy_scale);
      const uint16x4_t B0 = vqmovn_u32(A0);
      const uint16x4_t B1 = vqmovn_u32(A1);
...
WEBP_TSAN_IGNORE_FUNCTION void WebPRescalerDspInitNEON(void) {
  // Using 31b fixed-point precision seems *slower* than the
  // C-version with 32b fixed-point precision. So we disable this
  // NEON version for now.
  WebPRescalerExportRowExpand = RescalerExportRowExpand;
  WebPRescalerExportRowShrink = RescalerExportRowShrink;
  (void)RescalerExportRowExpand;
