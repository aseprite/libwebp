...
static void ConvertBGRAToBGRSSE2(const uint32_t* src,
  ...
  // the last storel_pd below writes 8 bytes starting at offset 18
  while (dst + 26 <= end) {
    const __m128i bgra0 = _mm_loadu_si128(in++);     // bgra0|bgra1|bgra2|bgra3
    const __m128i bgra4 = _mm_loadu_si128(in++);     // bgra4|bgra5|bgra6|bgra7
    ...
    _mm_storel_pd((double*)(dst +  0), (__m128d)c0);
    _mm_storeh_pd((double*)(dst +  6), (__m128d)c0);
    _mm_storel_pd((double*)(dst + 12), (__m128d)c4);
    _mm_storeh_pd((double*)(dst + 18), (__m128d)c4);
    dst += 24;
    num_pixels -= 8;
  }
  // left-overs
