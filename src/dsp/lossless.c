...
static float GetPredictionCostCrossColorRed(
    int tile_y_offset, int tile_x_offset, int all_x_max, int all_y_max,
  ...
  while (max_green_to_red - min_green_to_red > 2) {
    if (eval_min) {
      cur_diff_min = GetPredictionCostCrossColorRed(
          tile_y_offset, tile_x_offset, all_x_max, all_y_max, xsize,
...
static void GetBestGreenRedToBlue(
  ...
  const int num_iters = (1 + (max_green_to_blue - min_green_to_blue) / step) *
  ...
  for (green_to_blue = min_green_to_blue;
    ...
    for (red_to_blue = min_red_to_blue;
      ...
      cur_diff = GetPredictionCostCrossColorBlue(
          tile_y_offset, tile_x_offset, all_x_max, all_y_max, xsize, prev_x,
...
static Multipliers GetBestColorTransformForTile(
  ...
  GetBestGreenToRed(tile_y_offset, tile_x_offset, all_x_max, all_y_max, xsize,
                    prev_x, prev_y, &accumulated_red_histo[0], argb, &best_tx);
  GetBestGreenRedToBlue(tile_y_offset, tile_x_offset, all_x_max, all_y_max,
