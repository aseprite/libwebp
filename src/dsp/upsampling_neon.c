...
#define CONVERT8(FMT, XSTEP, N, src_y, src_uv, out, cur_x) do {         \
  ...
  for (i = 0; i < N; i += 8) {                                          \
    ...
    const uint8x8_t u  = vld1_u8((src_uv) + i);                         \
    ...
    const uint8x8_t R = vqmovun_s16(vshrq_n_s16(R2, YUV_FIX2));         \
    const uint8x8_t G = vqmovun_s16(vshrq_n_s16(G4, YUV_FIX2));         \
    const uint8x8_t B = vqmovun_s16(vshrq_n_s16(B3, YUV_FIX2));         \
