...
static void GetBestGreenRedToBlue(
  ...
  for (iter = 0; iter < iters; ++iter) {
    ...
    for (axis = 0; axis < kGreenRedToBlueNumAxis; ++axis) {
      int green_to_blue_cur = offset[axis][0] * delta + green_to_blue_best;
      int red_to_blue_cur = offset[axis][1] * delta + red_to_blue_best;
      float cur_diff = GetPredictionCostCrossColorBlue(
          argb, stride, tile_width, tile_height, prev_x, prev_y,
