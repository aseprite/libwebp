...
//-----------------------------------------------------------------------------
// Convert spans of 32 pixels to various RGB format, for the fancy upsampler
...
static void YuvToRgbaRowSSE2(const uint8_t* y,
  ...
  int n;
  for (n = 0; n + 4 <= len; n += 4) {
    const __m128i tmp0_1 = VP8GetRGBA32b(y[0], u[0], v[0]);
    const __m128i tmp0_2 = VP8GetRGBA32b(y[1], u[0], v[0]);
    const __m128i tmp0_3 = VP8GetRGBA32b(y[2], u[1], v[1]);
    const __m128i tmp0_4 = VP8GetRGBA32b(y[3], u[1], v[1]);
    const __m128i tmp1_1 = _mm_packs_epi32(tmp0_1, tmp0_2);
    const __m128i tmp1_2 = _mm_packs_epi32(tmp0_3, tmp0_4);
    const __m128i tmp2 = _mm_packus_epi16(tmp1_1, tmp1_2);
    _mm_storeu_si128((__m128i*)dst, tmp2);
...
static void YuvToRgbRowSSE2(const uint8_t* y,
  ...
  for (n = 0; n + 2 < len; ++n) {   // we directly stomp the *dst memory
    VP8YuvToRgbSSE2(y[0], u[0], v[0], dst);
    dst += 3;
    ++y;
    u += (n & 1);
    v += (n & 1);
  ...
  // Last two pixels are special: we write in a tmp buffer before sending
  // to dst.
  VP8YuvToRgbSSE2(y[0], u[    0], v[    0], tmp + 0);
  VP8YuvToRgbSSE2(y[1], u[n & 1], v[n & 1], tmp + 3);
  memcpy(dst, tmp, 2 * 3);
}
...
static void YuvToBgrRowSSE2(const uint8_t* y,
  ...
  uint8_t* const tmp = (uint8_t*)((uintptr_t)(tmp0 + 15) & ~15);  // align
  for (n = 0; n + 2 < len; ++n) {   // we directly stomp the *dst memory
    VP8YuvToBgrSSE2(y[0], u[0], v[0], dst);
    dst += 3;
    ++y;
    u += (n & 1);
    v += (n & 1);
  }
  VP8YuvToBgrSSE2(y[0], u[    0], v[    0], tmp + 0);
  VP8YuvToBgrSSE2(y[1], u[n & 1], v[n & 1], tmp + 3);
  memcpy(dst, tmp, 2 * 3);
}
