...
static void ConvertYUVToRGB(const __m128i* const Y0,
  ...
  *B = _mm_srli_epi16(B2, 6);   // <- ...logical shift here!
}
...
static void YUVToRGB(const uint8_t* const y,
                     const uint8_t* const u,
                     const uint8_t* const v,
                     __m128i* const R, __m128i* const G, __m128i* const B) {
  const __m128i Y0 = Load_HI_16(y), U0 = Load_HI_16(u), V0 = Load_HI_16(v);
  ConvertYUVToRGB(&Y0, &U0, &V0, R, G, B);
}

// Convert 32 samples of YUV420 to R/G/B
static void Y4UVToRGB(const uint8_t* const y,
                      const uint8_t* const u,
                      const uint8_t* const v,
                      __m128i* const R, __m128i* const G, __m128i* const B) {
...
static WEBP_INLINE void PlanarTo24b(const uint8_t* src, uint8_t* dst) {
  ...
  // It's also endian-dependent but we're only targetting x86.
...
static void YuvToRgbRow(const uint8_t* y, const uint8_t* u, const uint8_t* v,
  ...
  for (n = 0; n + 32 <= len; n += 32, dst += 32 * 3) {
    uint8_t tmp[32 * 3];
    __m128i R, G, B;
    Y4UVToRGB(y +  0, u +  0, v +  0, &R, &G, &B);
    PackAndStore3(&R, &G, &B, tmp +  0);
    Y4UVToRGB(y +  8, u +  4, v +  4, &R, &G, &B);
    PackAndStore3(&R, &G, &B, tmp +  8);
    Y4UVToRGB(y + 16, u +  8, v +  8, &R, &G, &B);
    PackAndStore3(&R, &G, &B, tmp + 16);
    Y4UVToRGB(y + 24, u + 12, v + 12, &R, &G, &B);
