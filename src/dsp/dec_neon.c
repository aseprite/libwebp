...
static WEBP_INLINE void TrueMotion(uint8_t* dst, int size) {
  ...
  for (n = 0; n < N; ++n) {
    ...
    for (x = 0; x < N; ++x) {
      vst1_lane_u32((uint32_t*)(dst + 0 * BPS + 4 * x), r0_u32, x);
      vst1_lane_u32((uint32_t*)(dst + 1 * BPS + 4 * x), r1_u32, x);
      vst1_lane_u32((uint32_t*)(dst + 2 * BPS + 4 * x), r2_u32, x);
      vst1_lane_u32((uint32_t*)(dst + 3 * BPS + 4 * x), r3_u32, x);
    }
    dst += 4 * BPS;
  }
}
