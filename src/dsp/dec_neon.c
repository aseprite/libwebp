...
static void TransformDC(const int16_t* in, uint8_t* dst) {
...

  {
    ...
    dst23_s16 = vaddq_s16(dst23_s16, DC);
    {
      ...
      const uint8x8_t dst01_ = vqmovun_s16(dst01_s16);
      const uint8x8_t dst23_ = vqmovun_s16(dst23_s16);

      // Store the results.
      *(int*)(dst + 0 * BPS) = vget_lane_s32(vreinterpret_s32_u8(dst01_), 0);
      *(int*)(dst + 1 * BPS) = vget_lane_s32(vreinterpret_s32_u8(dst01_), 1);
      *(int*)(dst + 2 * BPS) = vget_lane_s32(vreinterpret_s32_u8(dst23_), 0);
      *(int*)(dst + 3 * BPS) = vget_lane_s32(vreinterpret_s32_u8(dst23_), 1);
    }
  }
}
