...
static WEBP_INLINE void Load8x4(const uint8_t* const src, int stride,
                                uint8x8_t* const p1, uint8x8_t* const p0,
                                uint8x8_t* const q0, uint8x8_t* const q1) {
  *p1 = vld1_u8(src - 2 * stride);
  *p0 = vld1_u8(src - 1 * stride);
  *q0 = vld1_u8(src + 0 * stride);
  *q1 = vld1_u8(src + 1 * stride);
}
...
static WEBP_INLINE void Store8x4x2(const uint8x16_t p1, const uint8x16_t p0,
  ...
  vst1_u8(u - 2 * stride, vget_low_u8(p1));
  vst1_u8(u - 1 * stride, vget_low_u8(p0));
  vst1_u8(u + 0 * stride, vget_low_u8(q0));
  vst1_u8(u + 1 * stride, vget_low_u8(q1));
  vst1_u8(v - 2 * stride, vget_high_u8(p1));
  vst1_u8(v - 1 * stride, vget_high_u8(p0));
  vst1_u8(v + 0 * stride, vget_high_u8(q0));
  vst1_u8(v + 1 * stride, vget_high_u8(q1));
}
...
static WEBP_INLINE void Store6x8x2(const uint8x16_t p2, const uint8x16_t p1,
                                   ...
                                   uint8_t* u, uint8_t* v,
                                   int stride) {
  const uint8x8x3_t u0 =
    {{vget_low_u8(p2), vget_low_u8(p1), vget_low_u8(p0)}};
  const uint8x8x3_t u1 =
    {{vget_low_u8(q0), vget_low_u8(q1), vget_low_u8(q2)}};
