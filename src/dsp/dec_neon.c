...
#define STORE_WHT(dst, col, row01, row23) do { \
  dst[ 0] = vgetq_lane_s32(row01.val[0], col); \
  dst[16] = vgetq_lane_s32(row01.val[1], col); \
  dst[32] = vgetq_lane_s32(row23.val[0], col); \
  dst[48] = vgetq_lane_s32(row23.val[1], col); \
} while (0) \
...
static void TransformWHT(const int16_t* in, int16_t* out) {
...

  {
    ...
    for (i = 0; i < 2; ++i) {
      tmp0.val[i] = vshrq_n_s32(tmp0.val[i], 3);
      tmp1.val[i] = vshrq_n_s32(tmp1.val[i], 3);
    }

    STORE_WHT(out, 0, tmp0, tmp1); out += 64;
    STORE_WHT(out, 1, tmp0, tmp1); out += 64;
    STORE_WHT(out, 2, tmp0, tmp1); out += 64;
