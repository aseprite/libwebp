...
static WEBP_INLINE uint8x8x4_t Load4x8(const uint8_t* src, int stride) {
  ...
  in.val[0] = vset_lane_u32(*(const uint32_t*)src, in.val[0], 0);  // a0 a1 a2 a3
  src += stride;
  in.val[1] = vset_lane_u32(*(const uint32_t*)src, in.val[1], 0);  // b0 b1 b2 b3
  src += stride;
  in.val[2] = vset_lane_u32(*(const uint32_t*)src, in.val[2], 0);  // c0 c1 c2 c3
  src += stride;
  in.val[3] = vset_lane_u32(*(const uint32_t*)src, in.val[3], 0);  // d0 d1 d2 d3
  src += stride;
  in.val[0] = vset_lane_u32(*(const uint32_t*)src, in.val[0], 1);  // e0 e1 e2 e3
  src += stride;
  in.val[1] = vset_lane_u32(*(const uint32_t*)src, in.val[1], 1);  // f0 f1 f2 f3
  src += stride;
  in.val[2] = vset_lane_u32(*(const uint32_t*)src, in.val[2], 1);  // g0 g1 g2 g3
  src += stride;
  in.val[3] = vset_lane_u32(*(const uint32_t*)src, in.val[3], 1);  // h0 h1 h2 h3
  src += stride;
  // out{4} =
...
static WEBP_INLINE void Load4x16(const uint8_t* src, int stride,
  ...
  src -= 2;
  in.val[0] = vsetq_lane_u32(*(const uint32_t*)src, in.val[0], 0);
  src += stride;
  in.val[1] = vsetq_lane_u32(*(const uint32_t*)src, in.val[1], 0);
  src += stride;
  in.val[2] = vsetq_lane_u32(*(const uint32_t*)src, in.val[2], 0);
  src += stride;
  in.val[3] = vsetq_lane_u32(*(const uint32_t*)src, in.val[3], 0);
  src += stride;
  in.val[0] = vsetq_lane_u32(*(const uint32_t*)src, in.val[0], 1);
  src += stride;
  in.val[1] = vsetq_lane_u32(*(const uint32_t*)src, in.val[1], 1);
  src += stride;
  in.val[2] = vsetq_lane_u32(*(const uint32_t*)src, in.val[2], 1);
  src += stride;
  in.val[3] = vsetq_lane_u32(*(const uint32_t*)src, in.val[3], 1);
  src += stride;
  in.val[0] = vsetq_lane_u32(*(const uint32_t*)src, in.val[0], 2);
  src += stride;
  in.val[1] = vsetq_lane_u32(*(const uint32_t*)src, in.val[1], 2);
  src += stride;
  in.val[2] = vsetq_lane_u32(*(const uint32_t*)src, in.val[2], 2);
  src += stride;
  in.val[3] = vsetq_lane_u32(*(const uint32_t*)src, in.val[3], 2);
  src += stride;
  in.val[0] = vsetq_lane_u32(*(const uint32_t*)src, in.val[0], 3);
  src += stride;
  in.val[1] = vsetq_lane_u32(*(const uint32_t*)src, in.val[1], 3);
  src += stride;
  in.val[2] = vsetq_lane_u32(*(const uint32_t*)src, in.val[2], 3);
  src += stride;
  in.val[3] = vsetq_lane_u32(*(const uint32_t*)src, in.val[3], 3);
  src += stride;
...
static WEBP_INLINE void Load8x16(const uint8_t* const src, int stride,
  ...
  // TODO(skal): merge the cascading call into a proper 8x8 transpose?
