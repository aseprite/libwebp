...
static void RescalerImportRowShrinkSSE2(WebPRescaler* const wrk,
  ...
  const __m128i mult0 = _mm_set1_epi16(wrk->x_sub);
  ...
  if (wrk->x_add > (wrk->x_sub << 7)) {
    return WebPRescalerImportRowShrinkC(wrk, src);
  }
  assert(!WebPRescalerInputDone(wrk));
  assert(!wrk->x_expand);
  for (x_in = x_out = 0; x_out < 4 * wrk->dst_width; x_out += 4) {
    __m128i base = zero;
    accum += wrk->x_add;
    while (accum > 0) {
      const __m128i A = _mm_cvtsi32_si128(*(int*)(src + x_in));
      base = _mm_unpacklo_epi8(A, zero);
      // To avoid overflow, we need: base * x_add / x_sub < 32768
      // => x_add < x_sub << 7. That's a 1/128 reduction ratio limit.
      sum = _mm_add_epi16(sum, base);
      accum -= wrk->x_sub;
      assert(x_in < wrk->src_width);
      x_in += 4;
    }
    { // Emit next horizontal pixel.
      const __m128i mult = _mm_set1_epi16(-accum);
      const __m128i frac0 = _mm_mullo_epi16(base, mult);  // 16b x16b -> 32b
      ...
      const __m128i F1 = _mm_srli_epi64(E1, WEBP_RESCALER_RFIX);
      const __m128i F2 = _mm_and_si128(E2, mask);
      sum = _mm_packs_epi32(_mm_or_si128(F1, F2), zero);
      _mm_storeu_si128((__m128i*)(wrk->frow + x_out), frow_out);
    }
  }
  assert(accum == 0);
}
