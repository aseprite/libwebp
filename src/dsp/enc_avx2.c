...
void SSE4x4x2(const uint8_t* a, __m256i* b, int64_t *D1, int64_t *D2);
void SSE4x4x2(const uint8_t* a, __m256i* b, int64_t *D1, int64_t *D2) {
  const __m256i zero = _mm256_setzero_si256();

  // Load values. Note that we read 8 pixels instead of 4,
  // but the a/b buffers are over-allocated to that effect.
  const __m256i a0 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&a[BPS * 0]));
  const __m256i a1 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&a[BPS * 1]));
  const __m256i a2 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&a[BPS * 2]));
  const __m256i a3 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&a[BPS * 3]));
  ...
  const __m256i b01 = _mm256_unpacklo_epi32(b[0], b[1]);
  const __m256i b23 = _mm256_unpacklo_epi32(b[2], b[3]);
  ...
  const __m256i sum2 = _mm256_add_epi32(sum0, sum1);
...
void ITransform4x2(const uint8_t* ref, const int16_t* in, __m256i* dst, int do_two);
// Does one or two inverse transforms.
void ITransform4x2(const uint8_t* ref, const int16_t* in, __m256i* dst,
                           int do_two) {
  ...
  // Add inverse transform to 'ref' and store.
  {
    ...
    if (do_two) {
      ...
      ref0 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&ref[0 * BPS]));
      ref1 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&ref[1 * BPS]));
      ref2 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&ref[2 * BPS]));
      ref3 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&ref[3 * BPS]));
      flt = _mm256_load_si256((__m256i const *)filter_8);
    } else {
      // Load four bytes/pixels per line.
      ref0 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[0 * BPS]));
      ref1 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[1 * BPS]));
      ref2 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[2 * BPS]));
      ref3 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[3 * BPS]));
      flt = _mm256_load_si256((__m256i const *)filter_4);
    }
    // Convert to 16b.
    ...
    // Store the results.
    dst[0] = _mm256_packus_epi16(ref0, ref0);
    dst[1] = _mm256_packus_epi16(ref1, ref1);
    dst[2] = _mm256_packus_epi16(ref2, ref2);
    dst[3] = _mm256_packus_epi16(ref3, ref3);
  }
}


void FTransform4x2(const uint8_t* src, const uint8_t* ref, int16_t* out);
void FTransform4x2(const uint8_t* src, const uint8_t* ref, int16_t* out) {
  const __m256i zero = _mm256_setzero_si256();
  ...
  // Difference between src and ref and initial transpose.
  {
    ...
    // the data of the src is being broadcast to the first lane and the second lane of the 256 bit register.
    // A floating point broadcast is being used here because versus to integer broadcast this broadcast 
    // is done from the memeory and not from a register (1 cycle vs 3 cycles)  
    const __m256i src0 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&src[0 * BPS]));
    const __m256i src1 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&src[1 * BPS]));
    const __m256i src2 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&src[2 * BPS]));
    const __m256i src3 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&src[3 * BPS]));

    // convert src to 16b.
    const __m256i flt = _mm256_load_si256((__m256i const *)filter_4);
    ...
    // Load ref
    // The first 8 element are being broadcast to the first lane and second lane of the 256 bit register.
    const __m256i ref0 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[0 * BPS]));
    const __m256i ref1 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[1 * BPS]));
    const __m256i ref2 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[2 * BPS]));
    const __m256i ref3 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&ref[3 * BPS]));
    ...
    const __m256i ref_0 = _mm256_shuffle_epi8(ref0, flt);
    const __m256i ref_1 = _mm256_shuffle_epi8(ref1, flt);
    const __m256i ref_2 = _mm256_shuffle_epi8(ref2, flt);
    const __m256i ref_3 = _mm256_shuffle_epi8(ref3, flt);
...
void TTransform4x2(const uint8_t* inA,const __m256i* inB,
...
void TTransform4x2(const uint8_t* inA,const __m256i* inB,
  ...
  // Load, combine and transpose inputs.
  {
    // A floating point broadcast is being used here because versus to integer broadcast this broadcast 
    // is done from the memeory and not from a register (1 cycle vs 3 cycles)  
    const __m256i inA_0 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&inA[BPS * 0]));
    const __m256i inA_1 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&inA[BPS * 1]));
    const __m256i inA_2 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&inA[BPS * 2]));
    const __m256i inA_3 = _mm256_castpd_si256(_mm256_broadcast_sd((double const *)&inA[BPS * 3]));

    
    // Combine inA and inB (we'll do two transforms in parallel).
    const __m256i inAB_0 = _mm256_unpacklo_epi8(inA_0, inB[0]);
    const __m256i inAB_1 = _mm256_unpacklo_epi8(inA_1, inB[1]);
    const __m256i inAB_2 = _mm256_unpacklo_epi8(inA_2, inB[2]);
    const __m256i inAB_3 = _mm256_unpacklo_epi8(inA_3, inB[3]);
  ...
  // Vertical pass and difference of weighted sums.
  {
    ...
    const __m256i w_0 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&w[0]));
    const __m256i w_8 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&w[8]));
...
void Disto4x4x2(const uint8_t* const a, const __m256i* b, 
                   const uint16_t* const w, int64_t* SD1, int64_t* SD2);
...
#define QFIX2 0
WEBP_INLINE void DoQuantizeBlock4x2(int16_t in[32], int16_t out[32],
                                       int shift,
...
WEBP_INLINE void DoQuantizeBlock4x2(int16_t in[32], int16_t out[32],
  ...
  const __m256i iq0 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->iq_[0]));
  const __m256i iq8 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->iq_[8]));
  const __m256i q0 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->q_[0]));
  const __m256i q8 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->q_[8]));
  ...
  if (sharpen != NULL) {
    const __m256i sharpen0 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&sharpen[0]));
    const __m256i sharpen8 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&sharpen[8]));
  ...
  // out = (coeff * iQ + B) >> (QFIX + QFIX2 - shift)
  {
    ...
    // out = (coeff * iQ + B)
    const __m256i bias_00 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->bias_[0]));
    const __m256i bias_04 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->bias_[4]));
    const __m256i bias_08 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->bias_[8]));
    const __m256i bias_12 = _mm256_castpd_si256(_mm256_broadcast_pd((__m128d const *)&mtx->bias_[12]));
...
void QuantizeBlock4x2(int16_t in[32], int16_t out[32],
                  const VP8Matrix* const mtx,
...
void VP8CopyFromReg4x4(const __m256i* src, int lane, uint8_t* dst)
{
   // Copy either from the first lane or second lane 4 bytes.
   if (lane == 16)
   {
     *((int32_t *)&dst[0 * BPS]) = _mm_cvtsi128_si32(_mm256_extractf128_si256(src[0],1));
     *((int32_t *)&dst[1 * BPS]) = _mm_cvtsi128_si32(_mm256_extractf128_si256(src[1],1));
     *((int32_t *)&dst[2 * BPS]) = _mm_cvtsi128_si32(_mm256_extractf128_si256(src[2],1));
     *((int32_t *)&dst[3 * BPS]) = _mm_cvtsi128_si32(_mm256_extractf128_si256(src[3],1));
   }
   else
   {
     *((int32_t *)&dst[0 * BPS]) = _mm_cvtsi128_si32(_mm256_castsi256_si128(src[0]));
     *((int32_t *)&dst[1 * BPS]) = _mm_cvtsi128_si32(_mm256_castsi256_si128(src[1]));
     *((int32_t *)&dst[2 * BPS]) = _mm_cvtsi128_si32(_mm256_castsi256_si128(src[2]));
     *((int32_t *)&dst[3 * BPS]) = _mm_cvtsi128_si32(_mm256_castsi256_si128(src[3]));
   }
}
