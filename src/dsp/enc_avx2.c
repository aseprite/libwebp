...
static void SSE4x4x2(const uint8_t* a, const uint8_t* b, int64_t *D1, int64_t *D2) {
  ...
  const __m256i a0 = BROADCAST64(a[BPS * 0]);
  const __m256i a1 = BROADCAST64(a[BPS * 1]);
  const __m256i a2 = BROADCAST64(a[BPS * 2]);
  const __m256i a3 = BROADCAST64(a[BPS * 3]);
...
static void FTransform4x2(const uint8_t* src, const uint8_t* ref, int16_t* out) {
  ...
  // Difference between src and ref and initial transpose.
  {
    ...
    const __m256i src0 = BROADCAST64(src[0 * BPS]);
    const __m256i src1 = BROADCAST64(src[1 * BPS]);
    const __m256i src2 = BROADCAST64(src[2 * BPS]);
    const __m256i src3 = BROADCAST64(src[3 * BPS]);
    ...
    const __m256i ref0 = BROADCAST64(ref[0 * BPS]);
    const __m256i ref1 = BROADCAST64(ref[1 * BPS]);
    const __m256i ref2 = BROADCAST64(ref[2 * BPS]);
    const __m256i ref3 = BROADCAST64(ref[3 * BPS]);
...
static void TTransform4x2(const uint8_t* inA,const uint8_t* inB,
  ...
  // Load, combine and transpose inputs.
  {
    ...
    // and not from a register (1 cycle vs 3 cycles)
    const __m256i inA_0 = BROADCAST64(inA[BPS * 0]);
    const __m256i inA_1 = BROADCAST64(inA[BPS * 1]);
    const __m256i inA_2 = BROADCAST64(inA[BPS * 2]);
    const __m256i inA_3 = BROADCAST64(inA[BPS * 3]);
  ...
  {
    // Load all inputs.
    const __m256i w_0 = BROADCAST128(w[0]);
    const __m256i w_8 = BROADCAST128(w[8]);
...
static WEBP_INLINE void DoQuantizeBlock4x2(int16_t in[32], int16_t out[32],
  ...
  const __m256i iq0 = BROADCAST128(mtx->iq_[0]);
  const __m256i iq8 = BROADCAST128(mtx->iq_[8]);
  const __m256i q0 = BROADCAST128(mtx->q_[0]);
  const __m256i q8 = BROADCAST128(mtx->q_[8]);
  ...
  if (sharpen != NULL) {
    const __m256i sharpen0 = BROADCAST128(sharpen[0]);
    const __m256i sharpen8 = BROADCAST128(sharpen[8]);
  ...
  // out = (coeff * iQ + B) >> (QFIX + QFIX2 - shift)
  {
    ...
    // out = (coeff * iQ + B)
    const __m256i bias_00 = BROADCAST128(mtx->bias_[0]);
    const __m256i bias_04 = BROADCAST128(mtx->bias_[4]);
    const __m256i bias_08 = BROADCAST128(mtx->bias_[8]);
    const __m256i bias_12 = BROADCAST128(mtx->bias_[12]);
