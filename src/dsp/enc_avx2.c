...
static void SSE4x4x2(const uint8_t* a, const uint8_t* b, int64_t *D1, int64_t *D2) {
  ...
  const __m256i b0 = _mm256_load_si256((__m256i const *)(b + 0));
  const __m256i b1 = _mm256_load_si256((__m256i const *)(b + 32));
  const __m256i b2 = _mm256_load_si256((__m256i const *)(b + 32*2));
  const __m256i b3 = _mm256_load_si256((__m256i const *)(b + 32*3));
...
static void FTransform4x2(const uint8_t* src, const uint8_t* ref, int16_t* out) {
  ...
  // Difference between src and ref and initial transpose.
  {
    ...
    // versus to integer broadcast this broadcas is done from the memeory
...
static WEBP_INLINE void DoQuantizeBlock4x2(int16_t in[32], int16_t out[32],
  ...
  // out = (coeff * iQ + B) >> (QFIX + QFIX2 - shift)
  {
    ...
    // out = (coeff * iQ + B)
    const __m256i bias_00 = _mm256_castpd_si256(_mm256_broadcast_pd(
                                               (__m128d const *)&mtx->bias_[0]));
    const __m256i bias_04 = _mm256_castpd_si256(_mm256_broadcast_pd(
                                               (__m128d const *)&mtx->bias_[4]));
    const __m256i bias_08 = _mm256_castpd_si256(_mm256_broadcast_pd(
                                               (__m128d const *)&mtx->bias_[8]));
    const __m256i bias_12 = _mm256_castpd_si256(_mm256_broadcast_pd(
                                               (__m128d const *)&mtx->bias_[12]));
...
static void Copy4x4x2(const uint8_t* src, uint8_t* dst) { Copy(src, dst, 4); }
