...
double VP8LHistogramEstimateBits(const VP8LHistogram* const p) {
  return
      VP8LPopulationCost(p->literal_,
                         VP8LHistogramNumCodes(p->palette_code_bits_))
      + VP8LPopulationCost(p->red_, 256)
      + VP8LPopulationCost(p->blue_, 256)
      + VP8LPopulationCost(p->alpha_, 256)
      + VP8LPopulationCost(p->distance_, NUM_DISTANCE_CODES)
...
double VP8LHistogramEstimateBitsBulk(const VP8LHistogram* const p) {
  return
      ...
      + VP8LBitsEntropy(p->red_, 256)
      + VP8LBitsEntropy(p->blue_, 256)
      + VP8LBitsEntropy(p->alpha_, 256)
      + VP8LBitsEntropy(p->distance_, NUM_DISTANCE_CODES)
      + VP8LExtraCost(p->literal_ + 256, NUM_LENGTH_CODES)
      + VP8LExtraCost(p->distance_, NUM_DISTANCE_CODES);
}
...
static int GetCombinedHistogramEntropy(const VP8LHistogram* const a,
  ...
  *cost += VP8LGetCombinedEntropy(a->red_, b->red_, 256);
  if (*cost > cost_threshold) return 0;

  *cost += VP8LGetCombinedEntropy(a->blue_, b->blue_, 256);
  if (*cost > cost_threshold) return 0;

  *cost += VP8LGetCombinedEntropy(a->alpha_, b->alpha_, 256);
  if (*cost > cost_threshold) return 0;
...
static void UpdateHistogramCost(VP8LHistogram* const h) {
  const double alpha_cost = VP8LPopulationCost(h->alpha_, 256);
  const double distance_cost =
      VP8LPopulationCost(h->distance_, NUM_DISTANCE_CODES) +
      VP8LExtraCost(h->distance_, NUM_DISTANCE_CODES);
  const int num_codes = VP8LHistogramNumCodes(h->palette_code_bits_);
  h->literal_cost_ = VP8LPopulationCost(h->literal_, num_codes) +
                     VP8LExtraCost(h->literal_ + 256, NUM_LENGTH_CODES);
  h->red_cost_ = VP8LPopulationCost(h->red_, 256);
  h->blue_cost_ = VP8LPopulationCost(h->blue_, 256);
