...
static int PickBestIntra4(struct VP8EncIterator* const it, struct VP8ModeScore* const rd) {
  const VP8Encoder* const enc = it->enc_;
  const VP8SegmentInfo* const dqm = &enc->dqm_[it->mb_->segment_];
  ...
  do {
    ...
    Copy4x4x2((bestblock_reg_ptr+best_lane), best_blocks + VP8Scan[it->i4_]);
