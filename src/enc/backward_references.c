...
static const uint8_t plane_to_code_lut[128] = {
 97,   74,  56,  40,  24,  14,   6,   2,   0,   0,   0,   0,   0,   0,   0,   0,
 102,  79,  59,  43,  27,  17,   9,   3,   1,   4,  10,  18,  28,  44,  60,  80,
 103,  87,  63,  47,  33,  21,  11,   7,   5,   8,  12,  22,  34,  48,  64,  88,
 106,  91,  71,  53,  38,  29,  19,  15,  13,  16,  20,  30,  39,  54,  72,  92,
 111, 100,  83,  67,  49,  36,  31,  25,  23,  26,  32,  37,  50,  68,  84, 101,
 116, 109,  95,  77,  65,  51,  45,  41,  35,  42,  46,  52,  66,  78,  96, 110,
 119, 114, 104,  93,  81,  69,  61,  57,  55,  58,  62,  70,  82,  94, 105, 115,
 120, 117, 112, 107,  98,  89,  85,  75,  73,  76,  86,  90,  99, 108, 113, 118
};
...
static int DistanceToPlaneCode(int xsize, int dist) {
  ...
  if (xoffset <= 8 && yoffset < 8) {
    code = plane_to_code_lut[yoffset * 16 + 8 - xoffset];
  } else if (xoffset > xsize - 8 && yoffset < 7) {
    code = plane_to_code_lut[(yoffset + 1) * 16 + 8 + (xsize - xoffset)];
  } else {
    code = dist + 120;
  }
  assert(code > 0);
