...
static void ClearBackwardRefs(VP8LBackwardRefs* const refs) {
  assert(refs != NULL);
  *refs->trail_ = refs->free_blocks_;  // recycle all blocks at once
  refs->free_blocks_ = refs->refs_;
  refs->trail_ = &refs->refs_;
...
void VP8LBackwardRefsClear(VP8LBackwardRefs* const refs) {
  ...
  while (refs->free_blocks_ != NULL) {
    PixOrCopyBlock* const next = refs->free_blocks_->next;
...
static PixOrCopyBlock* BackwardRefsNewBlock(VP8LBackwardRefs* const refs) {
  ...
  if (b == NULL) {   // allocate new memory chunk
    const size_t total_size = sizeof(*b) + refs->block_size_ * sizeof(*b->start);
...
static void BackwardRefsCursorAdd(VP8LBackwardRefs* const refs,
  ...
  if (b == NULL || b->size == refs->block_size_) {
