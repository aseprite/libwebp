...
int (*GetResidualCost)(int ctx0, const VP8Residual* const res) = NULL;

static int GetResidualCostC(int ctx0, const VP8Residual* const res) {
  int n = res->first;
...
static int GetResidualCostMIPS32(int ctx0, const VP8Residual* const res) {
  int n = res->first;
  // should be prob[VP8EncBands[n]], but it's equivalent for n=0 or 1
  ...
  "1:                                                                       \n\t"
   ...
   "2:                                                                      \n\t"
    ...
    : [const_2]"r"(const_2), [const_255]"r"(const_255), [const_max_level]"r"(const_max_level),
      [VP8EncBands]"r"(VP8EncBands), [VP8LevelFixedCosts]"r"(VP8LevelFixedCosts),
      [VP8EntropyCost]"r"(VP8EntropyCost),
      [res_cost]"r"(res_cost), [res_prob]"r"(res_prob),
      [res_last]"r"(res_last), [res_coeffs]"r"(res_coeffs)
    : "memory"
  );
...
void GetResidualCostInit(void) {
  if (GetResidualCost == NULL) {
    GetResidualCost = GetResidualCostC;
    if (VP8GetCPUInfo) {
#if defined(WEBP_USE_MIPS32)
      if (VP8GetCPUInfo(kMIPS32)) {
        GetResidualCost = GetResidualCostMIPS32;
      }
#endif
    }
  }
