...
#define HEADER_SIZE_ESTIMATE (15 + 20 + 10)      // TODO: fix better
...
static float Clamp(float min, float max, float v) {
...
static float ComputeNextQ(PassStats* const s) {
  ...
  if (s->is_first) {
    ...
    dq = (s->last_q - s->q) * (s->target - s->value) / (s->last_value - s->value);
  } else {
    dq = 0.;  // we're done?!
  }
  // Limit variable to avoid large swings.
  s->dq = Clamp(-30.f, 30.f, dq);
  s->last_q = s->q;
  s->last_value = s->value;
  s->q = Clamp(0.f, 100.f, s->q + s->dq);
  return s->q;
}
...
static uint64_t OneStatPass(VP8Encoder* const enc, VP8RDLevel rd_opt,
  ...
  if (s->do_size_search) {
    ...
    s->value = 10.* log10(255. * 255. * pixel_count / distortion);
  }
  return size_p0;
}
...
int VP8EncTokenLoop(VP8Encoder* const enc) {
  ...
  while (ok && num_pass_left-- > 0) {
    ...
    if (stats.do_size_search) {
      ...
      stats.value = 10.* log10(255. * 255. * pixel_count / distortion);
    }
