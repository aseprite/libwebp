...
static int TrellisQuantizeBlock(const VP8Encoder* const enc,
...

  {
    ...
    // to last + 1 (inclusive) without losing much if there's not that many
    // non-zero coeff.
    if (last < 5) ++last;
    ...
    if (last == 0) {   // no coeff! Let's use a fast path to inspect coeff #0
      // hypothesis #1: block is skipped
      const score_t score1 = best_score;
      // hypothesis #2: we code the block as {1} only
      const score_t rate2 = (ctx0 == 0) ? VP8BitCost(1, last_proba) : 0;
      const score_t score2 = RDScoreTrellis(lambda, rate2, max_error);
      int last = 
