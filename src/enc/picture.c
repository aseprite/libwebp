...
#define MAKE_ARGB(r, g, b) (0xff000000u | (r << 16) | (g << 8) | b)
  ...
  if (!import_alpha) {
    ...
    for (y = 0; y < height; ++y) {
      for (x = 0; x < width; ++x) {
        const int offset = step * x + y * rgb_stride;
        const uint32_t argb =
            MAKE_ARGB(r_ptr[offset], g_ptr[offset], b_ptr[offset]);
...
void WebPBlendAlpha(WebPPicture* pic, uint32_t background_rgb) {
  ...
  if (!pic->use_argb) {
    ...
    for (y = 0; y < pic->height; ++y) {
      ...
      // Chroma blending every even lines
      if ((y & 1) == 0) {
        ...
        if (pic->width & 1) {   // rightmost pixel
          const int alpha = 2 * (a_ptr[2 * x + 0] + a_ptr2[2 * x]);
          u[x] = BLEND_10BIT(U0, u[x], alpha);
          v[x] = BLEND_10BIT(V0, v[x], alpha);
        }
      }
    ...
    const uint32_t background = MAKE_ARGB(red, green, blue);
    for (y = 0; y < pic->height; ++y) {
      for (x = 0; x < pic->width; ++x) {
        const int alpha = argb[x] >> 24;
        if (alpha != 0xff) {
          if (alpha > 0) {
