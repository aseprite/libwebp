...
static int MBAnalyzeBestIntra16Mode(VP8EncIterator* const it) {
  const int max_mode = (it->enc_->method_ >= 4) ? MAX_INTRA16_MODE : 4;
  int mode;
...
static int MBAnalyzeBestIntra4Mode(VP8EncIterator* const it,
                                   int best_alpha) {
  uint8_t modes[16];
  const int max_mode = (it->enc_->method_ >= 4) ? MAX_INTRA4_MODE : NUM_BMODES;
  int i4_alpha;
...
static int MBAnalyzeBestUVMode(VP8EncIterator* const it) {
  int best_alpha = DEFAULT_ALPHA;
  int best_mode = 0;
  const int max_mode = (it->enc_->method_ >= 4) ? MAX_UV_MODE : 4;
  int mode;
...
int VP8EncAnalyze(VP8Encoder* const enc) {
  ...
  const int do_segments =
      ...
      (enc->method_ == 0);  // for methods 0, we need preds_[] to be filled.
  enc->alpha_ = 0;
  enc->uv_alpha_ = 0;
  if (do_segments) {
