...
static WEBP_INLINE int Interpolate(int v) {
  ...
  assert(tab_pos + 1 <= kGammaTabSize);
  return y;
}
...
// This slower implementation is only 99.6% bit-exact with the table-based one
// above, given the 32bit-arithmetic restrictions on kAlphaFix.
...
static WEBP_INLINE void ConvertRowsToUVWithAlpha(const uint8_t* const r_ptr,
  ...
  for (i = 0, j = 0; i < (width >> 1); ++i, j += 2 * 4) {
    const uint32_t a = SUM4A(a_ptr + j);
    int r, g, b;
    if (a == 4 * 0xff || a == 0) {
      r = SUM4(r_ptr + j, 4);
      g = SUM4(g_ptr + j, 4);
  ...
  if (width & 1) {
    const uint32_t a = 2u * SUM2A(a_ptr + j);
    int r, g, b;
    if (a == 4 * 0xff || a == 0) {
      r = SUM2V(r_ptr + j);
      g = SUM2V(g_ptr + j);
